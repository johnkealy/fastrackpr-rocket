...
(function ($, Drupal, window, document) {
  ...
  Drupal.behaviors.rocketshipUIBase = {
    attach: function (context, settings) {
      ...
      if (skipLink.length && content.length) {
        ...
        once('skip-link', skipLink).forEach(function(linkElement) {
  ...
  self.cardLink = function(elements, linkSelector, fallbackSelector) {
    ...
    once('cardlink', elements).forEach(function(element) {
      var el = $(element),
      ...
      if (typeof link !== 'undefined' && link !== null && link.length) {
        ...
        once('cardlink-mousedown', el).forEach(function (el) {
        ...
        once('cardlink-mouseup', el).forEach(function (el) {
  ...
  self.scrollToAnchor = function(offset) {
    ...
    if (animatedScrolling) {
      ...
      if ( $(urlHash).length ) {
        ...
        once('scrollable-anchors-active', myAnchorLinks).forEach(function(linkElement) {
      ...
      once('scrollable-anchors', anchorLinks).forEach(function(linkElement) {
        ...
        once('scrollable-anchor-click', anchorLink).forEach(function (linkElement) {
      ...
      window.onpopstate = function(event) {
        once('scrollable-anchors-popstate', anchorLinks).forEach(function(linkElement) {
          var anchorLink = $(linkElement);
